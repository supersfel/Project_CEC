...
CalendarApp.prototype.openDayWindow = function (date) {
  ...
  const checkPlan1 = document.getElementById("plan1");
  const checkPlan2 = document.getElementById("plan2");
  const checkPlan3 = document.getElementById("plan3");

  if (day > today) {
    checkPlan1.disabled = false;
    checkPlan2.disabled = false;
    checkPlan3.disabled = false;
  } else if (
    y == today.getFullYear() &&
    m == today.getMonth() &&
    d == today.getDate()
    ...
    checkPlan1.disabled = true;
    checkPlan2.disabled = true;
    checkPlan3.disabled = true;
  }
  ...
  async function checkSchedule() {
    const getCheckScheduleApi = async () => {
      const res = await fetch("/auth/checkSchedule", {
        method: "POST",
        headers: {
    ...
    const res1 = await getCheckScheduleApi();
    if (JSON.parse(res1).plan1 >= 0) {
      checkPlan1.checked = JSON.parse(res1).plan1;
      checkPlan2.checked = JSON.parse(res1).plan2;
      checkPlan3.checked = JSON.parse(res1).plan3;
    } else {
      checkPlan1.checked = JSON.parse(res1).plan1;
      checkPlan2.checked = JSON.parse(res1).plan2;
      checkPlan3.checked = JSON.parse(res1).plan3;
...
CalendarApp.prototype.closeDayWindow = function () {
  ...
  const sendPlan1 = document.getElementById("plan1");
  const sendPlan2 = document.getElementById("plan2");
  const sendPlan3 = document.getElementById("plan3");
  let checkDate =
    day.getFullYear() +
    "" +
    ("00" + (day.getMonth() + 1)).slice(-2) +
  ...
  async function sendSchedule() {
    const getSendScheduleApi = async () => {
      const res = await fetch("/auth/sendSchedule", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          date: checkDate,
          plan1: sendPlan1.checked,
          plan2: sendPlan2.checked,
          plan3: sendPlan3.checked,
        }),
      });
    };
    getSendScheduleApi();
